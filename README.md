...
+ To build a Docker Image (this will create an image with `bhitsdev/pls:latest` tag):
...
A [SQL file](https://github.com/bhits-dev/pls/blob/master/pls-db-sample/pls_db_sample.sql) is provided with this project to populate it with a small set of sample provider data.
...
+ Run as a Docker Container: `docker run -d bhitsdev/pls:latest <additional program arguments>`
...
+ `docker run -d bhitsdev/pls:latest --server.port=80 --spring.datasource.password=strongpassword`
...
+ `docker run -d -v "/path/on/dockerhost/ssl_keystore.keystore:/path/to/ssl_keystore.keystore" bhitsdev/pls:latest --spring.profiles.active=ssl --server.ssl.key-store=/path/to/ssl_keystore.keystore --server.ssl.key-store-password=strongkeystorepassword`
...
Please use [GitHub Issues](https://github.com/bhits-dev/pls/issues) page to report issues.
